...
@router.post("/datasets/generate-ml-ready-files",
            summary="Generate ML-ready CSVs without DB",
            description="Runs the internal generator to write data/global_training_ml_ready.csv and data/local_validation_ml_ready.csv, overwriting existing files. Returns success/failure only.",
...
async def generate_ml_ready_files(
    ...
    try:
        # Ensure repo root is importable so we can import the experiments module
        repo_root = Path(__file__).resolve().parents[2]
        if str(repo_root) not in sys.path:
            sys.path.insert(0, str(repo_root))

        from experiments import generate_ml_ready_datasets as gen

        # Run the async generator inline so we can return a definitive status
        await gen.run(max_global=max_global, max_local=max_local, batch_size=batch_size, verbose=verbose)
        ...
        logger.error(f"Error generating ML-ready files: {e}")
