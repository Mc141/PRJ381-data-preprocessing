"""
Generate grid-based heatmaps for Pyracantha invasion risk prediction using REAL API DATA.
...
def load_model(model_file='model.pkl'):
    ...
    try:
        ...
        if not os.path.exists(model_path):
            ...
            print("python -m experiments.random_forest.train_model")
            sys.exit(1)
...
def create_heatmap(lats, lons, risk_scores, lat_grid, lon_grid, month, output_file):
    ...
    title_html = f'''
        <h3 align="center" style="font-size:18px; margin-top: 10px;">
            ...
            <span style="font-size:14px">{month_name} | Random Forest Model | REAL API DATA</span>
        </h3>
    '''
    ...
    stats_html = f'''
        <div style="position: fixed; 
            ...
            <table style="width:100%">
                ...
                <tr><td><b>Model:</b></td><td>Random Forest with REAL DATA</td></tr>
            </table>
    ...
    if not output_file:
        ...
        output_file = f"invasion_prediction_REAL_API_DATA_{timestamp}.html"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Generate Pyracantha invasion risk grid map for Western Cape using REAL API DATA.')
...
async def main_async():
    ...
    print("Starting invasion risk grid map generation pipeline with REAL API DATA...")
